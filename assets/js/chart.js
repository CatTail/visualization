...
Chart.prototype.render = function (data) {
  ...
  _this.container.selectAll('.dot')
    ...
    .attr('class', 'dot')
    .attr('r', 4)
    .attr('cx', function(d) { return _this.x(d.twitter) + 1.5; })
  ...
  this.container.selectAll('.dot').on('click', function () {
    _this.centre = {x: this.getAttribute('cx'), y: this.getAttribute('cy')};
    _this.socket.emit('graph');
  });
};
...
Chart.prototype.update = function (data) {
  //this.clear();
  this.graph.clear();
  this.graph.update(data, this.centre);
};
